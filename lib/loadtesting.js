...
requires stopping HTTP_SERVER. Use start(keepAlive=true) to not shut down HTTP_SERVER when done.

LoadTest contains members:
    ...
    - interval: statistics from this current interval
...
function requestGeneratorLoop(generator) {
    return function(finished, client) {
        ...
        client.on('error', function(err) {
            util.debug(err + '. ' + err.stack);
            // Setting the following gets client to call _reconnect, but it does so unsuccessfully.
            // When the connection becomes available, the next call to client.request() neither emits
            // 'response' nor 'error', so we never get to call callFinished().
            // client.readable = client.writable = false;
